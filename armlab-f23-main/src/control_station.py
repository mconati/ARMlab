...
from PyQt5.QtGui import QPixmap, QImage, QCursor, QMouseEvent
...
class Gui(QMainWindow):
    ...
    def __init__(self, parent=None, dh_config_file=None):
        ...
        self.ui.videoDisplay.mousePressEvent = self.mouseClickCallback
        self.cursorWorldCoordinate_mm = np.zeros(3, np.float64)
        ...
        self.ui.btnUser5.setText("Export Actions")
        self.ui.btnUser5.clicked.connect(lambda: self.sm.export_actions())
        self.ui.btnUser6.setText("Record Waypoint")
        self.ui.btnUser6.clicked.connect(lambda: self.sm.add_waypoint())
        self.ui.btnUser7.setText("Record Close Gripper Action")
        self.ui.btnUser7.clicked.connect(lambda: self.sm.add_close_gripper_action())
        self.ui.btnUser8.setText("Record Open Gripper Action")
        self.ui.btnUser8.clicked.connect(lambda: self.sm.add_open_gripper_action())
        self.ui.btnUser9.setText("Clear Actions")
        self.ui.btnUser9.clicked.connect(lambda: self.sm.clear_actions())
        self.ui.btnUser10.setText("Start Logging")
        self.ui.btnUser10.clicked.connect(lambda: self.sm.rxarm.start_logging())
        self.ui.btnUser11.setText("Stop Logging")
        self.ui.btnUser11.clicked.connect(lambda: self.sm.rxarm.stop_logging())
    ...
    def pixelToWorld(self, x: float, y: float, z: float) -> npt.NDArray[np.float64]:
        # precondition: self.camera.intrinsic_matrix is valid
        ...
        P_camera = z * np.linalg.inv(K) @ homogenous_pixels

        T_world_camera = np.linalg.inv(self.camera.extrinsic_matrix)
    ...
    def trackMouse(self, mouse_event):
        ...
        if self.camera.DepthFrameRaw.any() != 0:
            ...
            if self.camera.extrinsic_matrix is None:
                self.ui.rdoutMouseWorld.setText("ERROR: no extrinsic matrix")
            elif z == 0:
                self.ui.rdoutMouseWorld.setText("ERROR: depth=0")
            else:
                # convert from meters to millimeters
                self.cursorWorldCoordinate_mm = (
                    self.pixelToWorld(pt.x(), pt.y(), z / 1000) * 1000
                )
                self.cursorWorldCoordinate_mm[2] += self.camera.depth_correction[pt.y()]
                self.ui.rdoutMouseWorld.setText(
                    "(%.0f,%.0f,%.0f)"
                    % (
                        self.cursorWorldCoordinate_mm[0],
                        self.cursorWorldCoordinate_mm[1],
                        self.cursorWorldCoordinate_mm[2],
                    )
                )

    def mouseClickCallback(self, mouse_event: QMouseEvent):
        gripper_down = np.array([[1, 0, 0], [0, 0, 1], [0, -1, 0]])
        T_world_desired = np.eye(4)
        T_world_desired[0:3, 0:3] = gripper_down
        T_world_desired[:3, 3] = self.cursorWorldCoordinate_mm / 1000

        T_world_desired[2, 3] = np.maximum(0.03, T_world_desired[2, 3])

        target_angles = kinematics.IK_numerical_pox(
            T_world_desired, self.rxarm.M_matrix, self.rxarm.S_list
        )

        # TODO: we should really be calling a method of the state machine
        self.rxarm.set_positions(target_angles)

    def calibrateMousePress(self, mouse_event):
        # Not implemented; we automatically calibrate camera extrinsics based on
        # AprilTag detections.
        """!
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "-c", "--dhconfig", required=False, help="path to DH parameters csv file"
    )
