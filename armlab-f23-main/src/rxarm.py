...
sys.path.append(
    "../../interbotix_ws/src/interbotix_ros_toolboxes/interbotix_xs_toolbox/interbotix_xs_modules/interbotix_xs_modules/xs_robot"
)
...
def _ensure_initialized(func: Callable[[Any], Any]) -> Callable[[Any], Any]:
    """!
    ...
    def func_out(self, *args, **kwargs):  # type: ignore
        if self.initialized:
...
class RXArm(InterbotixManipulatorXS):
    ...
    def __init__(
        self,
        dh_config_file: Optional[str] = None,
        pox_config_file: Optional[str] = None,
    ):
        """!
        ...
        self.accel_time = 0.5
        # Feedback
        self.position_fb: list[float]
        self.velocity_fb: list[float]
        self.effort_fb: list[float]
        # DH Params
        self.dh_params: npt.NDArray[np.float64]
        self.dh_config_file = dh_config_file
        if dh_config_file is not None:
            self.dh_params = RXArm.parse_dh_param_file(dh_config_file)
        # POX params
        self.M_matrix: npt.NDArray[np.float64]
        self.S_list: list[npt.NDArray[np.float64]]
        if pox_config_file is not None:
            self.M_matrix, self.S_list = parse_pox_param_file(pox_config_file)

        self.log_file = "/tmp/do-not-use.csv"
        self.enable_logging = False

    def start_logging(self) -> None:
        if not self.enable_logging:
            self.log_file = "/tmp/" + time.strftime(
                "%Y-%m-%d-%H-%M-%S-joint-states.csv"
            )
    ...
    def log_positions(self) -> None:
        if self.enable_logging:
            with open(self.log_file, "a") as f:
                tokens = [f"{position:.6f}" for position in self.position_fb]
                tokens = [f"{time.time():.6f}"] + tokens
                line = ",".join(tokens)
                f.write(line + "\n")
    ...
    def initialize(self) -> bool:
        ...
        self.arm.go_to_home_pose(
            moving_time=self.moving_time, accel_time=self.accel_time, blocking=False
        )
    ...
    def sleep(self) -> None:
        ...
        self.accel_time = 1.0
        self.arm.go_to_home_pose(
            moving_time=self.moving_time, accel_time=self.accel_time, blocking=True
        )
    ...
    def set_positions(self, joint_positions: list[float]) -> None:
        """!
    ...
    def set_accel_time(self, accel_time: float) -> None:
    ...
    def get_ee_pose(self) -> list[float]:
        ...
        """
        T_world_endEffector = FK_pox(self.get_positions(), self.M_matrix, self.S_list)

        return get_pose_from_T(T_world_endEffector).tolist()  # type: ignore

    @_ensure_initialized
    def get_wrist_pose(self) -> list[float]:
        """!
    ...
    def parse_pox_param_file(self) -> int:
        """!
...
class RXArmThread(QThread):
    ...
    def __init__(self, rxarm: RXArm, parent: Optional[QObject] = None):
        """!
        @brief      Constructs a new instance.
        ...
        self.subscription = self.node.create_subscription(
            JointState, "/rx200/joint_states", self.callback, 10
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        rxarm.set_joint_positions(
            joint_positions, moving_time=2.0, accel_time=0.5, blocking=True
        )
        rxarm.gripper.grasp()
        rxarm.arm.go_to_home_pose()
